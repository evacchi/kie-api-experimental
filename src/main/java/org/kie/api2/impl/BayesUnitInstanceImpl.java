...
public class BayesUnitInstanceImpl<U extends BayesUnit> implements BayesUnitInstance<U> {
    ...
    private Method targetConstructor;
//
//    public BayesUnitInstanceImpl(U bu, InternalKnowledgeBase kBase) {
//        this(null);
//        unit = bu;
//    }

    public BayesUnitInstanceImpl(U unit, JunctionTree tree) {
        this.unit = unit;
        ...
        this.targetClass = (Class<U>) unit.getClass();
    ...
    public void setTargetClass(Class<U> targetClass) {
        this.targetClass = targetClass;
        buildParameterMapping(targetClass);
        buildFieldMappings(targetClass);
    }

    public void buildFieldMappings(Class<U> target) {
    ...
    public void buildParameterMapping(Class<U> target) {
...
//        }
        ...
        Method con = Arrays.stream(targetClass.getMethods()).filter(m -> m.getName().equals("update")).findFirst().get();
...
//        args[0] = this;
        for (int i = 0; i < targetParameterMap.length; i++) {
        ...
        try {
            targetConstructor.invoke(unit, args);
//            return unit;
            //return targetConstructor.newInstance(args);
        } catch (Exception e) {
            throw new RuntimeException("Unable to update " + targetClass.getSimpleName() + " " + Arrays.asList(args), e);
        }
    }
